...
class CartController extends Controller
{
    ...
    function check_pay()
    {
        ...
        if (isset($_POST)) {
            $result = $this->cartModel->donhang($_POST);
            if (isset($_SESSION['user'])) {
                $result2 = $this->cartModel->getItemInCart($_SESSION['user']['id']);
            }
            foreach($result2 as $index => $item){
                $check=$this->cartModel->chitietdonhang($result['id_donhang'], $item['id'], $item['ten'], $item['soluong'], $item['gia']*0.95*$item['soluong']);
                var_dump($check);
            }

            $resutl3=$this->cartModel->deleteCart($_SESSION['user']['id']);
