          ...
          <?php foreach ($data['item'] as $index => $item) : ?>
            <tr class="">
              ...
              <td>
                ...
                <div id="cart-amount" class="cart-amount"><?=$item['soluong']?></div>
                <button onclick="changeAmount(<?= isset($_SESSION['user']) ?  $_SESSION['user']['id'] : 0 ?>,<?= $item['id'] ?>,<?=$item['soluong']+1?>) changeAmountReload()" class="btn"><i class="fas fa-plus"></i></button>
              </td>
