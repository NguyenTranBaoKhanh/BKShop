<div class="wrapper bodymin">
          ...
          <tbody class="">
            ...
            <?php foreach ($data['item'] as $index => $item) : ?>
              <tr class="">
                ...
                <td class="text-center"><button onclick="deleteItem1(<?= $_SESSION['user']['id'] ?>,<?= $item['id'] ?>)">Xóa</button></td>
              </tr>
  ...
  <script>
    function changeAmount() {
      ...
      var totalNumber = 0;
      if (total != undefined) {
        for (var i = 0; i < script <?= count($data['item']) ?>; i++) {
    ...
    function deleteItem1(id_user, id_hanghoa) {
      ...
      if (check == true) {
        ...
        xhttp.open("GET", `${documentRoot}/cart/removecart/${id_user}/${id_hanghoa}`, true);
        xhttp.send();
      }
    }
  </script>
