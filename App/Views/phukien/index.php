<div class="content">
    <div class="container">
        ...
        <div class=" row row-cols-5 items">
            <?php if (!empty($data['smartphone'])) : ?>

                <?php foreach ($data['smartphone'] as $index => $item) : ?>
                    <div class="col item">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    <li class="page-item <?= $data['page'] <= 1 ? 'disabled' : '' ?>">
                        <a <?= $data['page'] <= 1 ? 'onclick="event.preventDefault()"' : '' ?> class="page-link" href="<?= DOCUMENT_ROOT . "/dienthoai?page=" . ($data['page'] - 1) ?>" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <?php for ($i = 1; $i <= $data['numPage']; $i++) : ?>
                        <li class="page-item <?= $i == $data['page'] ? 'page-active' : '' ?>"><a class="page-link" href="<?= DOCUMENT_ROOT . "/dienthoai?page=$i" ?>"><?= $i ?></a></li>
                    <?php endfor; ?>
                    <li class="page-item <?= $data['page'] == $data['numPage'] ? 'disabled' : '' ?>">
                        <a <?= $data['page'] == $data['numPage'] ? 'onclick="event.preventDefault()"' : '' ?> class="page-link" href="<?= DOCUMENT_ROOT . "/dienthoai?page=" . ($data['page'] + 1) ?>" aria-label="Next">
